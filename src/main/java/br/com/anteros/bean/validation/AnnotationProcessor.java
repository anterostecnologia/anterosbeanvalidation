...
public final class AnnotationProcessor {
	 ...
	 * @param access   strategy for <code>prop</code>
...
//		}
	 ...
	 *                          classes for <code>annotation</code>
	...
	private <A extends Annotation> boolean applyConstraint(A annotation,
		...
		final ConstraintValidator<A, ?> validator = getConstraintValidator(annotation, constraintClasses, owner,
				access);
		final AnnotationConstraintBuilder<A> builder = new AnnotationConstraintBuilder<A>(constraintClasses, validator,
	...
	private <A extends Annotation, T> ConstraintValidator<A, ? super T> getConstraintValidator(A annotation,
		...
		if (constraintClasses != null && constraintClasses.length > 0) {
			 ...
			 */
			Map<Type, Class<? extends ConstraintValidator<A, ?>>> validatorTypes = getValidatorsTypes(
					constraintClasses);
			...
			@SuppressWarnings("unchecked")
			final ConstraintValidator<A, ? super T> validator = (ConstraintValidator<A, ? super T>) factoryContext
	...
	private static <A extends Annotation> Map<Type, Class<? extends ConstraintValidator<A, ?>>> getValidatorsTypes(
		...
		Map<Type, Class<? extends ConstraintValidator<A, ?>>> validatorsTypes = new HashMap<Type, Class<? extends ConstraintValidator<A, ?>>>();
		for (Class<? extends ConstraintValidator<A, ?>> validatorType : constraintValidatorClasses) {
			Map<TypeVariable<?>, Type> typeArguments = TypeUtils.getTypeArguments(validatorType,
